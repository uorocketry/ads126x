    ...
    GpioPin: OutputPin,
{
    ...
    /// Returns the status register, the data, and the CRC.
    pub fn read_data1<SPI>(&mut self, spi: &mut SPI) -> Result<(register::StatusRegister, i32, u8)>
    where
        SPI: embedded_hal::blocking::spi::Write<u8> + embedded_hal::blocking::spi::Transfer<u8>,
    {
        ...
        spi.transfer(&mut buffer).map_err(|_| ADS126xError::IO)?;
        info!("buffer {:#010b}", buffer.clone());
        let data_buffer: [u8; 4] = [buffer[1], buffer[2], buffer[3], buffer[4]];
        let data: i32 = i32::from_be_bytes(data_buffer);
        let status =
            register::StatusRegister::from_bits(buffer[0]).expect("Invalid status register");
        Ok((status, data, buffer[5]))
    }
        ...
        SPI: embedded_hal::blocking::spi::Write<u8> + embedded_hal::blocking::spi::Transfer<u8>,
    {
        ...
        // let mut buffer = [0x20 | reg as u8];
        let mut buffer = [0x20 | reg.clone() as u8, 0x00, 0x20 | reg as u8, 0x00];
        // info!("Reading register {:X}", buffer[0]);
        // spi.write(&mut buffer).map_err(|_| ADS126xError::IO)?;
        // buffer = [0x00];
        spi.transfer(&mut buffer).map_err(|_| ADS126xError::IO)?;
        info!("Read register {:#010b}", buffer);
        Ok(buffer[2])
    }
    ...
    where
        SPI: embedded_hal::blocking::spi::Write<u8> + embedded_hal::blocking::spi::Transfer<u8>,
    {
        let mut buffer = [0x40 | reg as u8, 0x00, data];
        // spi.transfer(&mut buffer).map_err(|_| ADS126xError::IO)?;
        spi.write(&mut buffer).map_err(|_| ADS126xError::IO)?;
        Ok(())
    }
    ...
    where
        SPI: embedded_hal::blocking::spi::Write<u8> + embedded_hal::blocking::spi::Transfer<u8>,
    {
        info!("reg as u8 {:X}", Register::INPMUX as u8);
